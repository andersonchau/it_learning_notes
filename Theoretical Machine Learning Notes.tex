...
In practice, computation using K O(d) instead of directly from \(\phi(x)\) is much faster. Further, We only need to know K but only know the existence of \(\phi(x)\) and no need to be able to write down. Consider the Kernel applied to bitmap : number of bits as d. (Great reduction!)
\newline  
...
\newline 
Application : To SVM, perceptron, linear regression, and other learning algorithm represented only in inner product \(\langle x,z\rangle\) , then Apply K(x,z)  
\end{document}
